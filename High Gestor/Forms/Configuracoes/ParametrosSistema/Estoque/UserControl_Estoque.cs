...
{
    public partial class UserControl_Estoque : UserControl
    {
        #region Dll
        ...
        {
            string query = ("SELECT bloquearVendaEstoque FROM ParametrosSistema");
            ...
            if (datareader.Read())
            {
                comboBoxBloquearVendas.Text = datareader.GetString(0);
            }
            banco.desconectar();
        }
        ...
        {
            string query = ("UPDATE ParametrosSistema SET bloquearVendaEstoque = @value, idLog = @idLog, updatedAt = @updatedAt");
            SqlCommand exeQuery = new SqlCommand(query, banco.connection);

            exeQuery.Parameters.AddWithValue("@value", comboBoxBloquearVendas.Text);
        ...
        private void UserControl_Estoque_Load(object sender, EventArgs e)
        {
            carregarDados();
        }
