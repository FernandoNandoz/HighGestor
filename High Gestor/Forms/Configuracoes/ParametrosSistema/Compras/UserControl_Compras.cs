...
{
    public partial class UserControl_Compras : UserControl
    {
        #region Dll
        ...
        {
            string query = ("SELECT atualizarCustoProduto FROM ParametrosSistema");
            ...
            if (datareader.Read())
            {
                comboBoxAtualizarCusto.Text = datareader.GetString(0);
            }
            banco.desconectar();
        }
        ...
        {
            string query = ("UPDATE ParametrosSistema SET atualizarCustoProduto = @value, idLog = @idLog, updatedAt = @updatedAt");
            SqlCommand exeQuery = new SqlCommand(query, banco.connection);

            exeQuery.Parameters.AddWithValue("@value", comboBoxAtualizarCusto.Text);
        ...
        private void UserControl_Compras_Load(object sender, EventArgs e)
        {
            carregarDados();
        }
