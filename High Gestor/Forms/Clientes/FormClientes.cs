...
namespace High_Gestor.Forms.Clientes
{
    public partial class FormClientes : Form
    {
        ...
        private void dataCliente()
        {
            ...
            string Produtos = ("SELECT idCliente, nomeCompleto_RazaoSocial, CPF_CNPJ, situacao, tipoPessoa FROM Clientes ORDER BY idCliente");
            ...
            while (datareader.Read())
            {
                if (datareader.GetString(4) == "JURIDICA")
                {
                    CPF_CNPJ = FormatCNPJ(datareader.GetString(2));
                }
                else if (datareader.GetString(4) == "FISICA")
                {
        ...
        private void buttonPesquisar_Click(object sender, EventArgs e)
        {
            ...
            if (textBoxPesquisar.Text != string.Empty)
            {
                ...
                {
                    //Retorna os dados da tabela Produtos para o DataGridView
                    string Produtos = ("SELECT idCliente, nomeCompleto_RazaoSocial, CPF_CNPJ, situacao, tipoPessoa FROM Clientes WHERE idCliente LIKE (@ID + '%') ORDER BY idCliente");
                    SqlCommand exeVerificacao = new SqlCommand(Produtos, banco.connection);
                    banco.conectar();
                ...
                {
                    //Retorna os dados da tabela Produtos para o DataGridView
                    string Produtos = ("SELECT idCliente, nomeCompleto_RazaoSocial, CPF_CNPJ, situacao, tipoPessoa FROM Clientes WHERE nomeCompleto_RazaoSocial LIKE (@nome_razao + '%') OR nomeFantasia LIKE (@nomeFantasia + '%') ORDER BY idCliente");
                    SqlCommand exeVerificacao = new SqlCommand(Produtos, banco.connection);
                    banco.conectar();
