...
namespace High_Gestor.Forms.Clientes
{
    public partial class FormCadCliente : Form
    {
        ...
        private void carregarDados()
        {
            ...
            string query = ("SELECT idCliente, situacao, tipoPessoa, nomeCompleto_RazaoSocial, nomeFantasia, dataNascimento, responsavel, CPF_CNPJ, telefoneWhatsApp, telefoneContato, email, enderecoCompleto, complemento, observacao FROM Clientes WHERE idCliente = @ID");
            ...
            while (datareader.Read())
            {
                ...
                maskedWhatsApp.Text = datareader.GetString(8);
                maskedTelefoneContato.Text = datareader.GetString(9);
                textBoxEmail.Text = datareader.GetString(10);
                textBoxEndereco.Text = datareader.GetString(11);
                textBoxComplemento.Text = datareader.GetString(12);
                textBoxObservacao.Text = datareader.GetString(13);
            }
        ...
        private bool verificarClienteExistente()
        {
            ...
            if (datareader.Read())
            {
                ...
                if (textBoxNomeFantasia.Text == datareader[1].ToString())
                {
                    message = message + "A Nome Fantasia informado já existe." + "\n";
                }

                if (maskedCPF_CNPJ.Text == datareader[2].ToString())
                {
        ...
        private void insertQuery()
        {
            try
            {
                string query = ("INSERT INTO Clientes (situacao, tipoPessoa, nomeCompleto_RazaoSocial, nomeFantasia, responsavel, dataNascimento, CPF_CNPJ, telefoneWhatsApp, telefoneContato, email, enderecoCompleto, complemento, observacao, idLog, createdAt) VALUES (@situacao, @tipoPessoa, @nomeCompleto_RazaoSocial, @nomeFantasia, @responsavel, @dataNascimento, @CPF_CNPJ, @telefoneWhatsApp, @telefoneContato, @email, @enderecoCompleto, @complemento, @observacao, @idLog, @createdAt)");
